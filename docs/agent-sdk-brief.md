...
*observes and governs* them. It reads each agent's **live API (REST + SSE)** and
...
- `GET /agent/interrupted` — open HITL gates (`waiting_for_input`)
...
async def handle_task(task):
    async with agent.run(work_item=task.id, type="spec",
        ...
        # ... do work ...
        decision = await run.request_approval(                          # opens a HITLGate, emits hitl.requested,
            prompt="Approve this spec?", channel="slack",              # blocks until resolved (or interrupt-resume)
            metadata={"completeness": 86},                             # domain data → metadata
        )
...
- `run.request_approval(...)` should integrate with the agent's **existing**
  ...
  gate is *represented and emitted*; it does not replace the agent's HITL logic.
...
```jsonc
{
  ...
  "skills": [ { "id": "generate_spec", "name": "Generate specification",
                "description": "Produce an EARS spec from a task" } ],
  "x-agency": {                         // our extension (A2A doesn't model pipeline topology)
    "id": "ba",
    "domain": "sdlc",
    "contractVersion": "0.1",
    "produces": ["spec"],
...
- Keep the agent's existing HITL/runtime logic — the SDK standardizes
  *representation + emission*, not the agent's brain.
