...
Agency OS is the operating system for **agent–human SDLC pods**: a curated team of AI agents (BA · SA · UI/UX · Developer · QA · DevOps/Release · Knowledge) paired with **one accountable human per agent**, delivering software through a governed, gated, auditable pipeline. A client-side PM **fires up** a pod (picks agents, connects Teamwork/Slack/GitHub, assigns accountable people), **runs** it day-to-day (gates, incidents, comms), and **monitors** it (ROI, governance, SLA, compliance). One-liner: *not an autonomous coder you must trust — a governed agent–human pipeline you can prove.*
...
**BA repo** (`BA_Agent`): the one **live** agent. `docs/platform-overview.md` (how BA works), `ONBOARDING.md` (dev onboarding), `packages/agency-agent-sdk/` (the SDK BA serves the contract through). BA's API is the reference producer implementation.
