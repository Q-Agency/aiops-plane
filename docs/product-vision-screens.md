...
- **Interactions:**
  ...
  - **Reject back-arrow on review cards = a decision, not a move:** opens a typed-reason dialog (min 10 chars, the reject canon) → ticket returns to the previous agent stage + `gate.rejected "{from} → {to}: {reason}"` lands on the session ledger; toast "sent back — reason recorded · your note becomes the agent's added context on the rerun."
...
- **Persona & entry:** Pod Admin / PM. Entered from the **Pipeline header action** ("Work intake", Inbox icon — never "add"), and from the product's empty-state CTAs (Command Center empty, Pipeline empty Backlog, post-launch "warming up" ribbon) which deep-link here as the "where work arrives" surface.
...
- **States:**
  - *not connected (gate):* `alert` "Connect Teamwork or Jira — work arrives from your board." + CTA to Connect; copy points at the FIRE UP readiness check (no paste fallback exists).
...
- **States:** This *is* the empty state. Per surface the copy differs (every add-work CTA below deep-links to **Work Intake** — `/intake`):
  - Command Center → "Pod is live but quiet — no active work yet." CTA "Open Work intake" → `/intake` (work ARRIVES from the board — never "add").
