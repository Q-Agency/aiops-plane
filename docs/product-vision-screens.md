...
- **Copy — the exact rail (route → pillar migration baked in):**
  ...
  - **RUN** — `Command Center` → `/` · `Pipeline` → `/pipeline` · `Gates` → `/approvals` (badge: open gates) · `Comms & Escalations` → `/comms` · `Incidents & Recovery` → `/incidents` (NEW; badge: open incidents).
  - **MONITOR** — `Overview / ROI` → `/economics` (REFRAME hero) · `Governance` → `/governance` · `Accountability` → `/pod` (note: shared route with People & Roles; same matrix, MONITOR framing) · `SLA & Reports` → `/reports` (NEW) · `Compliance & Audit` → `/compliance` · `Usage & Billing` → `/billing` (NEW).
  - **ADVANCED** (collapsed by default) — `Observability` → `/observability` · `Orchestration` → `/orchestration` · `Flow` → `/flow` · `Traceability` → `/traceability` · `Agents` → `/agents` (deep-dive lives here).
...
- **Demo note:** The reorganized rail is the visual proof that the product is FIRE UP → RUN → MONITOR, not a 16-tab engineering tour. The presenter reads top-to-bottom to narrate the arc before clicking anything.

> **Route-to-pillar migration note (for the IA reorg):** `agents/agents.$agentId/observability/orchestration/flow/traceability` → ADVANCED (demoted, REUSE). `index→Command Center`, `pipeline`, `approvals→Gates`, `comms` → RUN (REUSE). `economics→Overview/ROI`, `governance`, `pod→Accountability`, `compliance` → MONITOR (REUSE/REFRAME). `connections`, `pod→People&Roles`, `knowledge` (fold under People & Roles or Catalog detail) → FIRE UP (REFRAME). NEW routes to scaffold: `/pods/new`, `/catalog`, `/incidents`, `/reports`, `/billing`. `pod` is intentionally referenced from two pillars (People & Roles in FIRE UP, Accountability in MONITOR) — same component, different entry framing; no duplication of the route.
...
- **Demo note:** Establishes "a client org runs several pods" in one glance; the presenter switches from a live pod to the freshly-launched demo pod right after the FIRE UP Launch step, proving the switch re-scopes everything.
...
- **States:** *Pod Admin/PM* → `/` (Command Center). *Engineering Lead* → `/pipeline`. *QA Lead* → `/approvals` (Gates). *Client Sponsor/Exec (read-only)* → `/economics` (Overview/ROI digest). *Viewer* → `/economics` (status digest, read-only). *No-pod edge-case* → `/pods/new` regardless of role (except Viewer → a "no pod" empty state).
...
- **Demo note:** Switch role → Sponsor lands on the ROI hero (the wow number), reinforcing "what did my money buy?"; switch back to Admin lands on the Command Center cockpit.
...
- **Layout:** Takes over the `<main>` region inside the existing `AppShell` (TopBar + LeftRail stay; tenancy badge stays visible — fire-up happens *inside* an already-provisioned tenant, per vision §2). Three-row content grid:
  - **Wizard header (row 1, sticky):** left = title "Fire up a new pod" + working pod name (inline-editable, defaults "Untitled Pod"); right = "Save & exit" (ghost) + auto-save indicator ("Saved · 2s ago").
...
- **Interactions:**
  ...
  - "Save & exit" persists the draft, toasts "Draft saved — resume from New Pod", and routes to the pod switcher / Command Center.
  - On the final step, "Launch pod" is the gate (owned by the Readiness cluster) — it transitions the draft to a live pod and routes to `/` (Command Center) with a success toast.
- **Copy:** Header "Fire up a new pod". Step labels: "1 Blueprint · 2 Agents · 3 Connect · 4 People · 5 Slack · 6 Go live". Autosave "Saved · {n}s ago". Disabled-Continue tooltip "Finish this step to continue". Save&exit confirm "Your draft is saved — you can resume any time from FIRE UP → New Pod." Resume banner "Pick up where you left off?".
- **Demo note:** This is the frame for the 0:00–0:30 FIRE UP beat — the "30 seconds that does not exist yet." The stepper visibly fills as the presenter clicks through Blueprint → Agents → Connect → People → Slack → Go live, making the "fire up a pod in minutes" claim legible on screen.
...
- **Demo note:** This is **0:00–0:30 FIRE UP**, beat 1 of 3. The presenter adds **BA + SA + UI/UX + Dev + QA** with five switch taps; adding Dev surfaces the inline `+ Add Tasklist` auto-suggest and adding QA surfaces `+ Add Review` — **five taps + two one-click auto-fixes** complete the continuous 7-node wired chain (no dashed-amber gaps) — the "assemble a delivery team in 30 seconds" moment. The `Live`/`Roadmap` honesty and the contract badge pre-empt the lock-in pushback inline.
...
- **Demo note:** The brownfield beat inside FIRE UP — the presenter scopes the pod to the `pod-owned` label, the preview narrows live to 6 tickets, and the banner lands the coexistence line: "everything else stays with your team, untouched." Pre-empts the "will it eat our board?" objection before Launch.
...
- **Persona & entry:** Pod Admin / PM, from left rail FIRE UP → "Connections", or deep-linked from an Incidents "tool-disconnected" item, or from a degraded health badge anywhere.
...
- **Copy:** Title "Launching **AutoMarket** pod…" · steps as above · final "**Pod is live.** Taking you to Command Center…" · error "Launch halted — {reason}. Nothing was lost; your draft is saved."
- **Demo note:** The ~2-second satisfying beat that bridges FIRE UP → RUN. Keep it short — it must not steal time from the RUN/MONITOR acts. The presenter clicks Launch and is on the Command Center within ~2s.

---

### Post-Launch Landing (Command Center, first-run framing) — `/`  (REFRAME)
- **Purpose:** Where the freshly launched pod lands — the existing Command Center, but with a **first-run welcome ribbon** so the PM sees "your pod is live, here's what it's doing" instead of a cold dashboard. Closes the FIRE UP→RUN loop.
...
- **States:**
  ...
  - **Returning visit:** ribbon absent (dismissed flag persisted, e.g. cookie/localStorage keyed by pod id); standard Command Center.
  - **Loading / error / populated:** inherited from `RealCommandCenter` (its `EmptyFleet`, polling, `LiveBadge`) — no new states.
- **Mock data:** Inherits the Command Center's existing data path (`getCommandCenterFn`, mock fleet/runs/approvals). The ribbon reads pod name + the just-launched summary counts from the `PodDraft` in `src/mock/fireup.ts` (agents/humans/tools/slack counts). The "warming up" empty path uses the existing `LifecycleFlow` zero-state.
...
- **Demo note:** The seam where the demo crosses from **FIRE UP into RUN (0:30)**. The ribbon makes the cut feel intentional — "you just built this pod, now watch it work" — and the presenter clicks **View pipeline** (or lets the first clarification gate appear in the approvals aside) to begin Act II.
...
### Command Center (reframed) — `/index`  (REFRAME)
- **Purpose:** The RUN landing — the live cockpit where a PM keeps a pod running: see ROI at a glance, scan agent liveness-vs-activity, clear human gates, watch the stream, and *take control* (pause pod / pause agent) when something looks wrong.
- **Persona & entry:** Pod Admin / PM (primary). Lands here by default on sign-in and after **Launch pod** from the FIRE UP wizard. Engineering/QA leads pass through on the way to Pipeline/Gates. The TopBar pod-switcher scopes everything on this page to one pod.
...
- **Interactions:**
  ...
  - **ROI hero click:** deep-links to **Overview / ROI** (`/economics`) — the MONITOR landing — preserving the demo's "promoted here, full story there" framing.
...
- **Copy:**
  ...
  - Empty fleet (reframed): "No pod running yet — fire up your first pod." CTA button: "New Pod" → wizard.
...
- **Persona & entry:** Pod Admin/PM and Engineering Lead. Entry: RUN section of the left rail, from the Command Center pipeline KPI tile, or a Cmd-K deep-link to a ticket.
...
- **Components:**
  ...
  - shadcn primitives to fold in (currently hand-rolled — swap for consistency): `dialog` (replace the hand-built confirm overlay), `tooltip` (the reject back-arrow hint, age chips), `scroll-area` (the board horizontal scroll), `badge` (codebase/priority/rerun chips), `toggle-group` (Day/Night).
  - NEW small additions: a **"Open review →" affordance** on cards sitting in any `*-review` column that deep-links into the Gate Review surface (see Spec Review screen) rather than only allowing drag-to-approve; a **filter bar** (codebase, priority, accountable human, overnight-eligible) above the board to match the IA's "list-filtering on high-volume views."
- **States:**
  - *empty (new pod):* board renders all 14 columns with a single dashed "No work yet — add a ticket or connect Teamwork/Jira" card in Backlog; CTA "Add work →" deep-links to **Work Intake** (`/intake`, specced below).
...
- **Interactions:**
  - **Drag-to-dispatch is CUT (locked decision):** cards are **not** draggable into agent columns — pipeline stages advance only via **real agent runs** (the dashboard is a control plane, not an orchestrator; no dispatch op exists in the contract). When an upstream run completes, the card advances on its own with the running shimmer + auto-advance animation.
  - Drag card to a **review column** (the only permitted drag target) → opens the approve-gate `dialog` (REFRAME: this confirm should instead route to the client-grade Gate Review surface for Spec/Design, keeping the lightweight dialog only as quick-confirm).
  - Reject back-arrow on review cards → returns ticket to the previous agent stage (increments `rerunCount`), toast "returned with feedback."
  - Click a card title → opens the Gate Review surface (for review stages) or Traceability (for non-review stages).
- **Copy:** Title "Pipeline Board"; helper "Tickets flow left → right. Agent columns advance only when the agent's real run completes; review columns are human gates — the only drag target." Empty Backlog: "No work yet." Overnight banner: "Overnight run scheduled 22:00 UTC."
- **Demo note:** 0:30 RUN beat — a Teamwork/Jira ticket lands in Backlog → flows to Ready for Spec → BA runs → lands in **Spec Review**; the PM clicks "Open review →" to jump to the client-grade surface.

---

### Work Intake — `/intake`  (NEW)
- **Purpose:** The missing work-intake surface — how tickets actually enter the pod: pick tickets from the connected Teamwork/Jira backlog (checkbox picker, pre-filtered by the pod's `scopeRule`), or paste one ticket by hand, with a per-ticket **routing preview** showing exactly which agent will take it first. The stage for the demo's 0:30 "a ticket flows in" beat — without it a fresh pod dead-ends after Launch.
- **Persona & entry:** Pod Admin / PM. Entered from the **Pipeline Backlog header action** ("+ Add work"), and — load-bearing — from **every empty-state CTA in the product**: each "Add a ticket / connect Jira"-class CTA (Command Center empty, Pipeline empty Backlog, Economics empty, post-launch "warming up" ribbon) deep-links here. Also reachable via ⌘K ("Add work").
- **Layout:** AppShell content, `max-w-5xl`. Two `tabs`: **From your tracker** and **Paste a ticket**.
  - *From your tracker:* a header row (connector chip "Teamwork · Connected" + the active `scopeRule` sentence with an "Edit scope" link) over a checkbox `table` of in-scope tickets (checkbox, id, title, labels, priority, age). A sticky footer bar: "{n} selected" + primary "Pull {n} tickets into the pod".
  - *Paste a ticket:* a minimal composer `card` — title `input`, description `textarea` (markdown ok), priority `select`, optional label chips — with a live **routing preview** beside it.
  - *Routing preview (both tabs):* a compact rail per selected/composed ticket: "AM-142 → **Backlog** → **Ready for Spec** → BA Agent runs first" — derived from `chain.ts` (the first pipeline agent whose `consumes` is empty, i.e. BA), so the buyer sees where work lands before committing.
- **Components:** shadcn: `tabs`, `table` + `checkbox` (picker), `input`/`textarea`/`select` (composer), `card`, `badge` (connector/label/priority chips), `button`, `alert` (not-connected gate), `scroll-area`, `tooltip`, `sonner` (pulled-in toast), `skeleton`. REUSE: connector identity + health from `connectors.ts`; ticket-row rendering from the Pipeline `TicketCard` (compact); the chain ordering from `chain.ts`. NEW: `WorkIntakeView`, `TicketPickerTable`, `TicketComposer`, `RoutingPreview`.
- **States:**
  - *not connected (gate):* if no ticketing connector is connected, the tracker tab shows an `alert` "Connect Teamwork or Jira to pull tickets" + CTA to Connect (wizard step 3 / Connections Hub); the **Paste a ticket** tab stays fully usable — a pod is never dead-ended.
  - *loading:* picker `skeleton` rows while the (mocked) backlog fetch resolves.
  - *populated:* in-scope tickets listed; already-pulled tickets render greyed with an "In pod" badge (no double-pull).
  - *empty (scope matches nothing):* "Your scope matches no open tickets — widen it or paste one." + "Edit scope" link.
  - *error:* destructive `alert` "Couldn't reach Teamwork — retry" (paste tab unaffected).
  - *edge — composing an invalid ticket:* "Pull into pod" disabled until title non-empty; helper "A one-line title is enough — the BA asks for the rest."
- **Mock data:** NEW `src/mock/intake.ts`: `intakeBacklog: IntakeTicket[]` (`{ id; title; labels: string[]; priority; ageDays; inScope: boolean; pulled: boolean }`, ~10 rows from the client-backlog extension in `tickets.ts`, **seeded with `AM-142`** at the top — the canonical demo ticket) + `pullTickets(ids: string[])` / `composeTicket(draft)` helpers that append to `tickets.ts` Backlog and fire a `notifications.ts` entry; routing preview computed from `chain.ts` (no stored data).
- **Interactions:** Check tickets → footer counts up → "Pull {n} tickets into the pod" → rows animate out, toast "AM-142 pulled into AutoMarket pod — routing to BA", tickets appear in the Pipeline Backlog, the bell increments, and an `intake.pulled`-class entry goes to the audit bridge. Composer "Pull into pod" does the same for the pasted ticket. "Edit scope" → the Connect sub-screen 3b. Routing preview rows deep-link to `/pipeline?ticket=<id>`.
- **Copy:** Title "Work intake". Sub "Pull work from your tracker, or paste a ticket — the pod routes it from there." Tabs "From your tracker / Paste a ticket". Scope sentence "Showing your pod's scope: AutoMarket · label `pod-owned`." Footer CTA "Pull {n} tickets into the pod". Not-connected alert "Connect Teamwork or Jira to pull tickets — or paste one by hand." Toast "{id} pulled — routing to BA Agent."
- **Demo note:** The stage for **0:30 RUN** — immediately after Launch, the presenter opens Work Intake (or lets the Demo Director's `seedInflow` do it), checks **AM-142**, clicks "Pull 1 ticket into the pod", and cuts to the Pipeline as the card lands in Backlog and the BA picks it up. Every empty state in the product points here, so a fresh pod is never a dead end.
...
- **Persona & entry:** Pod Admin / PM lands from the left rail (RUN → Comms & Escalations) or by deep-link from an escalation badge on the Command Center / a bell notification. QA/Eng leads enter to confirm an escalation routed to them was acknowledged.
...
### MONITOR — Overview / ROI — `/economics` (REFRAME)
...
- **Persona & entry:** Pod Admin/PM lands here from the left rail (MONITOR group, item 1) or from the Command Center ROI hero tile's "View ROI →" deep-link. Client Sponsor/Exec lands here as their rolled-up surface. Read-only-friendly; no operator dials.
...
- **States:**
  ...
  - **Edge — empty accountability column:** if an agent has no accountable human (`OWNERSHIP` gap), the matrix cell renders the existing "uncovered risk" red flag and the drawer is disabled with "Assign an accountable human first" (links to FIRE UP People & Roles).
...
- **Layout:** A compact draggable overlay panel (`Card`, ~320px, corner-pinned, high z-index, dimmed-glass) that floats above any screen without stealing focus. Header: "Demo Director" + script name + elapsed timer + auto/manual `toggle`. Body: the **step list** mirroring the 3-minute script (0:00 FIRE UP → 0:30 ticket inflow + clarification → 1:30 escalation/incident → 1:45 MONITOR reveal → 2:45 moat close), each row: timestamp, label, state dot (pending / fired / current). Footer: `▶ Run` / `⏸ Pause`, `Step →` (manual advance), and `Reset to checkpoint` (select + button).
...
- **Purpose:** Send each user to the right default surface on login instead of dumping everyone on the operator Command Center — the spine that makes the multi-stakeholder story real.
...
- **Demo note:** Optional opener flourish — log in as PM (Command Center), then "Switch view" to Sponsor to reveal the exec digest, proving one pod, many audiences. Sets up the 1:45 MONITOR beat.
...
- **Persona & entry:** Pod Admin / PM. Entry from LeftRail "People & Roles" (FIRE UP pillar) and from the FIRE UP wizard's People step (shares this component). Read-only roles see the matrix without the invite/edit controls.
...
- **States:**
  ...
  - *empty (no members):* "No people on this pod yet — invite your team" CTA (this is the FIRE UP setup entry).
...
- **Layout:** Identical to the normal Command Center (`RealCommandCenter`) — the point is it looks real. The only differences: a **"Sample" badge** in the TopBar next to the pod name and a dismissible `glass-panel` ribbon under the TopBar offering "This is a sample pod — explore freely. Create your own pod →".
...
- **Interactions:** "Create your own pod →" → New Pod wizard (FIRE UP). "Reset sample" restores seed. Everything else behaves like the live Command Center (gates approvable, etc.) but mutations are local-only and resettable. The pod switcher lists "AutoMarket (Sample)" with the badge.
...
- **Demo note:** This is the cleanest demo substrate — the entire 3-min arc can run on the Sample pod with zero setup risk; the "Sample" badge keeps it honest while showing the rich populated experience. The "Create your own pod →" CTA is the bridge into the FIRE UP 0:00 beat.
...
- **Components:** shadcn `card`/`glass-panel`, `button`, lucide icon. NEW `EmptyState` component: `EmptyState({ icon, title, body, cta?: { label, to } })`. Drop into Command Center, Pipeline, Gates, Comms, Incidents, Economics, Governance, SLA & Reports.
- **States:** This *is* the empty state. Per surface the copy differs (every add-work CTA below deep-links to **Work Intake** — `/intake`):
  - Command Center → "Pod is live but quiet — no active work yet." CTA "Add work" → `/intake`.
  - Pipeline → "Nothing in the pipeline. Work appears here once a ticket enters." CTA "Add work" → `/intake`.
...
- **Interactions:** CTA routes to the relevant FIRE UP setup step, to **Work Intake** (`/intake`) for every add-work CTA, or loads the sample pod. Empty states with no sensible action (e.g. "All clear — no incidents") have no CTA, just calm confirmation.
- **Copy:** As listed per surface above; tone is calm/explanatory, never error-like. Default CTA on data-bearing views "Load sample pod" or "Add work" (→ `/intake`).
...
- **Purpose:** For read-only personas, hide operator pillars (FIRE UP, RUN controls) and surface only MONITOR read surfaces — so a sponsor never sees a "Launch pod" or "Approve" affordance they can't use.
- **Persona & entry:** Sponsor / Viewer, on every screen. PM/leads see the full pillar-grouped rail.
- **Layout:** Same `LeftRail` aside, but the `items` list is filtered by role: Sponsor/Viewer get only Overview/ROI, Governance (read), Accountability (read), SLA & Reports, Compliance & Audit. FIRE UP, Pipeline controls, Gates-as-actor, Incidents, Settings-config are removed; Settings shows posture-only.
...
- **Interactions:** Rail renders role-scoped items; hidden routes guard-redirect. Pillar group headers (FIRE UP / RUN / MONITOR / ADVANCED) are the same ones the IA reorg introduces.
...
**IA spine: fully covered.** Every left-rail item in §6 has a spec. Verified item-by-item: FIRE UP (New Pod wizard, Catalog, Connections, People & Roles) ✓; RUN (Command Center, Pipeline, Gates, Comms & Escalations, Incidents & Recovery) ✓; MONITOR (Overview/ROI, Governance, Accountability, SLA & Reports, Compliance & Audit, Usage & Billing) ✓; Settings ✓; TopBar (Pod switcher, Tenancy badge, ⌘K, Bell, user/role) ✓.
...
17. **[LOW] EmptyState component:** xcut-roles defines a global `EmptyState({icon,title,body,cta})`. Many clusters hand-roll empty copy (Command Center "EmptyFleet", Pipeline dashed card, Gates "All clear", etc.). **Action: name `EmptyState` as the canonical primitive and have all cluster empty-states use it** — otherwise you get N bespoke empty cards.

18. **[LOW] ROI hero forked.** run-command-center defines `RoiHeroTile`; monitor-roi-billing reuses `RoiHeadline` + defines `HeroKpi`. The Command Center hero and the `/economics` hero should share one component. **Name one (`RoiHero`/`HeroKpi`) and reuse on both.**
...
##### `src/mock/intake.ts` — Work Intake (`/intake`)
```ts
export interface IntakeTicket { id: string; title: string; labels: string[]; priority: "P0"|"P1"|"P2"|"P3"; ageDays: number; inScope: boolean; pulled: boolean; }
export const intakeBacklog: IntakeTicket[];   // ~10 rows from the client-backlog extension; AM-142 seeded on top
export function pullTickets(ids: string[]): void;     // appends to tickets.ts Backlog + fires a notifications.ts entry + audit-bridge `intake.pulled`
export function composeTicket(draft: { title: string; description?: string; priority?: string; labels?: string[] }): IntakeTicket;
...
```ts
export const NAV: { pillar: "FIRE UP" | "RUN" | "MONITOR" | "ADVANCED"; items: { to: string; label: string; icon: string; minCapability?: keyof Role["capabilities"]; badgeKey?: "gates" | "incidents" }[] }[];
