...
- **Copy — the exact rail (route → pillar migration baked in):**
  ...
  - **RUN** — `Overview` → `/` · `Agents` → `/agents` *(PROMOTED from ADVANCED 2026-06-12 — the roster is the product's central object, not a technical deep-dive; deep per-agent diagnostics still live on the drill-in page)* · `Pipeline` → `/pipeline` · `Work Intake` → `/intake` · `Gates` → `/approvals` (badge: open gates) · `Comms & Escalations` → `/comms` (badge: unacked escalations) · `Incidents & Recovery` → `/incidents` (badge: open incidents).
  ...
  - **ADVANCED** (collapsed by default — a pure platform/SRE drawer) — `Observability` → `/observability` · `Orchestration` → `/orchestration` · `Platform status` → `/status`. *(Agents/Flow/Traceability were re-placed to RUN/MONITOR on 2026-06-12 — see above.)*
