...
- **Copy — the exact rail (route → pillar migration baked in):**
  ...
  - **RUN** — `Overview` → `/` · `Pipeline` → `/pipeline` · `Gates` → `/approvals` (badge: open gates) · `Comms & Escalations` → `/comms` · `Incidents & Recovery` → `/incidents` (NEW; badge: open incidents).
  - **MONITOR** — `ROI & Economics` → `/economics` (REFRAME hero) · `Governance` → `/governance` · `Accountability` → `/pod` (note: shared route with People & Roles; same matrix, MONITOR framing) · `SLA & Reports` → `/reports` (NEW) · `Compliance & Audit` → `/compliance` · `Usage & Billing` → `/billing` (NEW).
...
> **Route-to-pillar migration note (for the IA reorg):** `agents/agents.$agentId/observability/orchestration/flow/traceability` → ADVANCED (demoted, REUSE). `index→Overview`, `pipeline`, `approvals→Gates`, `comms` → RUN (REUSE). `economics→Overview/ROI`, `governance`, `pod→Accountability`, `compliance` → MONITOR (REUSE/REFRAME). `connections`, `pod→People&Roles`, `knowledge` (fold under People & Roles or Catalog detail) → LAUNCH (REFRAME). NEW routes to scaffold: `/pods/new`, `/catalog`, `/incidents`, `/reports`, `/billing`. `pod` is intentionally referenced from two pillars (People & Roles in LAUNCH, Accountability in MONITOR) — same component, different entry framing; no duplication of the route.
