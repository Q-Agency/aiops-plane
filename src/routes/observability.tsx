...
import { RealObservabilityView } from "@/components/observability/RealObservabilityView";
...
export const Route = createFileRoute("/observability")({
  loader: async ({ context }) => {
    if (context.user?.dataMode === "real") {
      const diagnostics = await getObservabilityFn().catch(() => [] as AgentDiagnostics[]);
      return { mode: "real" as const, diagnostics };
    }
...
function ObservabilityRoute() {
  ...
  return d.mode === "real" ? (
    <RealObservabilityView initial={d.diagnostics} />
  ) : (
    <ObservabilityView />
  );
}
    ...
    <section className="space-y-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KpiTile
          ...
          value={`$${(totalToday * 6.4).toFixed(0)}`}
          sub="9 days runway @ burn"
          tone="text-foreground"
        />
        <KpiTile
          label="Batch API savings"
          value={`$${(totalToday * 1.8).toFixed(2)}`}
          sub="overnight @ 50% off"
          tone="text-status-done"
          icon={Moon}
        />
        <KpiTile
          label="Local vLLM offload"
          value="38%"
          sub="of curator + tasklist runs"
          tone="text-agent-curator"
          icon={Cpu}
        />
      </div>
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.4fr_1fr] gap-3">
        <Panel
          title="Agent-API dispatch health"
          sub="last 24h · success / error rate per agent endpoint"
          icon={Network}
        >
          <div className="space-y-2">
            {dispatches.map((w) => (
              <div
                key={w.name}
                className="grid grid-cols-[1fr_60px_60px_80px] gap-2 items-center text-[11px] font-mono"
              >
                ...
                <span
                  className={cn(
                    "text-right",
                    w.err > 0 ? "text-status-error" : "text-muted-foreground",
                  )}
                >
                  {w.err}
                </span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {grafanaTiles.map((t) => (
              <div key={t.label} className="border border-border rounded p-2.5">
                ...
                <div className={cn("font-mono font-semibold text-base mt-0.5", t.tone)}>
                  {t.value}
                </div>
              </div>
            ))}
...
function OvernightReport() {
  ...
  return (
    <section className="space-y-3">
      <SectionHeader
        icon={Moon}
        title="Overnight run · last night"
        sub="22:00 → 06:00 UTC · 2×H200"
        tone="primary"
      />

      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <KpiTile
          label="Tickets attempted"
          value={String(lastNight.tickets)}
          tone="text-foreground"
          icon={Activity}
        />
        <KpiTile
          label="PRs created"
          value={String(lastNight.prs)}
          tone="text-status-done"
          icon={GitPullRequest}
        />
        <KpiTile
          label="Auto-QA pass rate"
          value={lastNight.testsPass}
          tone="text-status-done"
          icon={Zap}
        />
        <KpiTile
          label="Healer repairs"
          value={String(lastNight.healerFixes)}
          tone="text-status-waiting"
          icon={Wrench}
        />
        <KpiTile
          label="Total cost"
          value={`$${(lastNight.gpuHours * 4.2).toFixed(2)}`}
          sub={`${lastNight.gpuHours}h GPU`}
          tone="text-primary"
          icon={DollarSign}
        />
      </div>
...
function HealthTimeline() {
  ...
  const tone = (kind: string) =>
    kind === "agent error"
      ? "bg-status-error/70 border-status-error"
      : kind === "SLA breach"
        ? "bg-status-waiting/70 border-status-waiting"
        : "bg-agent-pm/70 border-agent-pm";
  const icon = (kind: string) =>
    kind === "agent error" ? AlertOctagon : kind === "SLA breach" ? AlertTriangle : Network;
  ...
  return (
    <section className="space-y-3">
      <SectionHeader
        ...
        sub="errors · SLA breaches · dispatch failures"
      />
          ...
          {incidents.map((inc, i) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "absolute top-2 bottom-2 rounded border flex items-center px-1.5 gap-1 cursor-help hover:brightness-125 transition",
                  tone(inc.kind),
                )}
                style={{
              ...
              >
                ...
                <span className="text-[10px] font-mono text-foreground truncate hidden md:inline">
                  {inc.kind}
                </span>
              </div>
            );
...
function TraceExplorer() {
  ...
  const [outcomeFilter, setOutcomeFilter] = useState<"all" | "approved" | "rejected" | "error">(
    "all",
  );
  ...
  return (
    <section className="space-y-3">
      <SectionHeader
        ...
        sub={`${filtered.length} of ${allRows.length} traces · Langfuse-style`}
      />
                  ...
                  <td>
                    <span className="inline-flex items-center gap-1.5">
                      <span
                        ...
                        style={{ background: r.agentColor, boxShadow: `0 0 6px ${r.agentColor}` }}
                      />
    ...
    <div className="flex items-end justify-between gap-3">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <div className={cn("text-base font-semibold", tone === "primary" && "text-primary")}>
            {title}
          </div>
        </div>
      </div>
